...
export const processMapIcon = async (chunithmOptionDirectory: string) => {
  ...
  return await Promise.all(mapIconDirectories.map(async mapIconDirectoryName => {
    const mapIconDirectory = path.join(mapIconBaseDirectory, mapIconDirectoryName)
    const mapIcon = await parser.parseStringPromise(fs.readFileSync(path.join(mapIconDirectory, 'MapIcon.xml')))
