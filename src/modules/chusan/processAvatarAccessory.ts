...
export const processAvatarAccessory = async (chunithmOptionDirectory: string) => {
  ...
  const avatarAccessoryBaseDirectory = path.join(chunithmOptionDirectory, 'avatarAccessory')
  ...
  return await Promise.all(avatarAccessoryDirectories.map(async avatarAccessoryDirectoryName => {
    const avatarAccessoryDirectory = path.join(avatarAccessoryBaseDirectory, avatarAccessoryDirectoryName)
    const avatarAccessory = await parser.parseStringPromise(fs.readFileSync(path.join(avatarAccessoryDirectory, 'AvatarAccessory.xml')))
