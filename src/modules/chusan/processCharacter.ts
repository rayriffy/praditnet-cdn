...
export const processCharacter = async (
  ...
  return await Promise.all(
    characterDirectories.map(async characterDirectoryName => {
      const characterDirectory = path.join(
        characterBaseDirectory,
        characterDirectoryName
      )
      const character = await parser.parseStringPromise(
        fs.readFileSync(path.join(characterDirectory, 'Chara.xml'))
      )
      ...
      if (
        ddsItems.find(
          o => o.name === character.CharaData.defaultImages[0].str[0]
        ) !== undefined
      ) {
        await Promise.all(
          Object.entries(
            ...
            if (!fs.existsSync(expectedFilePath)) {
              await promiseSpawn(
                'convert',
                [`'${value.fullPath}'`, `'${temporaryFilePath}'`],
                {
                  shell: true,
                }
              )
