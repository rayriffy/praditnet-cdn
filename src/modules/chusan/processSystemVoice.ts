...
export const processSystemVoice = async (chunithmOptionDirectory: string) => {
  ...
  const systemVoiceBaseDirectory = path.join(
  ...
  return await Promise.all(
    systemVoiceDirectories.map(async systemVoiceDirectoryName => {
      const systemVoiceDirectory = path.join(
        systemVoiceBaseDirectory,
        systemVoiceDirectoryName
      )
      const systemVoice = await parser.parseStringPromise(
        fs.readFileSync(path.join(systemVoiceDirectory, 'SystemVoice.xml'))
      )
       ...
       */
      const outputIconDirectory = path.join(
        publicDirectory,
        'chunithm',
        'systemVoice',
        'icon'
      ...
      const expectedIconFilePath = path.join(
        outputIconDirectory,
        `${payload.id}.png`
      )
      ...
      if (!fs.existsSync(expectedIconFilePath)) {
        ...
        await buildAsset(temporaryFilePath, expectedIconFilePath)
       ...
       * Get sample audio
       */
      const targetSystemVoiceSampleFilePath = path.join(
        extractorDirectory,
        'output',
        'chusan',
        'sound',
      ...
      const outputSystemVoiceSampleDirectory = path.join(
        publicDirectory,
        'chunithm',
        'systemVoice',
        'sample'
      )
      const expectedSystemVoiceSampleFilePath = path.join(
        outputSystemVoiceSampleDirectory,
        `${payload.id}.mp3`
      )
