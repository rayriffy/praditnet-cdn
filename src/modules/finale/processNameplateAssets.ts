...
export const processNameplateAssets = async () => {
  const nameplateDirectory = path.join(
    finaleDirectory,
    'maimai',
    'data',
    'sprite',
    'collection_nameplate'
  )
  const nameplateOutputDirectory = path.join(
    publicDirectory,
    'finale',
    'nameplate'
  )
  ...
  const qualifiedNameplateFiles = nameplateFiles.filter(
  ...
  for await (const nameplateFile of qualifiedNameplateFiles) {
    ...
    const temporaryFilePath = path.join(
      nameplateOutputDirectory,
      `${nameplateId}_TMP.png`
    )
    const convertedNameplatePath = path.join(
      nameplateOutputDirectory,
      `${nameplateId}.png`
    )
    ...
    if (!fs.existsSync(convertedNameplatePath)) {
      await promiseSpawn(
        'convert',
        [`'${nameplatePath}'`, `'${temporaryFilePath}'`],
        {
          shell: true,
        }
      )

      await buildAsset(temporaryFilePath, convertedNameplatePath)
